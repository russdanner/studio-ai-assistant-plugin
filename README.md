...
This repository is a **Crafter Studio** plugin that brings AI into authoring. It ships two main surfaces:

1. **Interactive chat** — The **Studio AI assistant** is available from **TinyMCE**, the **form-engine control**, **Experience Builder / preview**, and the optional **Helper** widget in the Tools Panel or preview toolbar. You define **agents** in `ui.xml` or widget JSON. Each agent selects an **LLM** and optional **tools**; the model may call built-in tools and return text for insert or review.

2. **Autonomous runs (experimental)** — Optional **Tools Panel** widget **`AutonomousAssistants`** runs scheduled, server-side steps per agent with supervisor logic and in-memory state. It reuses the interactive tool catalog where the chosen LLM supports it. Behavior, limits, and configuration: [docs/internals/spec.md](docs/internals/spec.md) § *Autonomous assistants widget*.

**LLM backends:** Each agent sets **`<llm>`** in site configuration. Supported providers, wire ids, secrets, defaults, and per-backend capabilities are documented in [docs/using-and-extending/llm-configuration.md](docs/using-and-extending/llm-configuration.md).

**Built-in tools:** The server exposes a function-tool catalog for sessions that support native Studio tools. Coverage includes CMS repository operations, HTTP helpers, optional CrafterQ consult and chat APIs, image generation when configured, and related behaviors described in [docs/using-and-extending/llm-configuration.md](docs/using-and-extending/llm-configuration.md) and [docs/internals/spec.md](docs/internals/spec.md). Which tools apply depends on the selected **`llm`**; use the capability matrix in **llm-configuration.md**.

**Custom tools:** Add Groovy tools under **`config/studio/scripts/aiassistant/user-tools/`** with a **`registry.json`** manifest. Security and layout follow the same rules as other Studio scripts. See [docs/using-and-extending/studio-plugins-guide.md](docs/using-and-extending/studio-plugins-guide.md).

**Developers:** For building and installing Crafter Studio plugins (plugin ID, paths, `ui.xml`, auth, Rollup, troubleshooting 404s), see [docs/using-and-extending/studio-plugins-guide.md](docs/using-and-extending/studio-plugins-guide.md).

**Documentation map:** [All documentation](docs/README.md) · [Configuration guide](docs/using-and-extending/configuration-guide.md) · [Using and extending](docs/using-and-extending/README.md) · [Plugin internals](docs/internals/README.md).
