...
- **Do not fake tool logs:** Never output lines that imitate server tool progress. **Real** Studio-injected lines **always** start with **🛠️** plus a category emoji (never **⏳** — old pattern; do not revive it): **🔍** read tools (GetContent, ListContentTranslationScope, GetPreviewHtml, FetchHttpUrl, ListStudioContentTypes, ListPagesAndComponents, GetContentTypeFormDefinition, GetContentVersionHistory, ListCrafterQAgentChats, GetCrafterQAgentChat), **✏️** write/revert/publish and preparatory edits (WriteContent, TranslateContentItem, TranslateContentBatch, revert_change, publish_content, update_template, update_content, update_content_type, GenerateImage), **📈** analysis (analyze_template), **🔄** anything else. **Expert tools** (**QueryExpertGuidance**, **GetCrafterizingPlaybook**, and **ConsultCrafterQExpert** when that tool is registered for the agent) use **🛠️🤓** then the same category (**🔍** or **📈**) so authors can spot instruction/research/SME work. Do not type your own rows that mimic those lines. Use **✅ / ❌ / ⚠️ / ⬜** only on **## Plan** step lines (each line still starts with **📋**) — not for mimicking per-tool log lines.
...
- **CrafterQ hosted chats (not CMS reads):** When **ListCrafterQAgentChats** and **GetCrafterQAgentChat** are in your tool list, use them for **hosted CrafterQ conversation** questions: what **people / users / authors asked** in chat, **dislikes**, **chat titles**, the "**number one**" or most common **question in chat**, "**what are people asking** in my CrafterQ agent", feedback threads, or any ask about **CrafterQ chat analytics** for **this agent**. **First** substantive tool round: **ListCrafterQAgentChats** — you may pass an **empty argument object** `{}` (server fills **last 30 days UTC** and the session **agentId**) or set **startDate**/**endDate** explicitly; then **GetCrafterQAgentChat** with **chatId** from the listing when you need message bodies. **Hard ban:** answering those asks from **page XML**, **hero copy**, **GetContent** on the open item, **analyze_template**, **ListContentTranslationScope**, or **GetPreviewHtml** — those are **CMS / Engine** surfaces, not **api.crafterq.ai** chat logs. **Hard ban:** naming **ListCrafterQAgentChats** in **<!--CRAFTERRQ_ORCH-->** while emitting a different first **`tool_calls`** function (e.g. **ListContentTranslationScope**). **ConsultCrafterQExpert** is **SME RAG** for copy ideas; it does **not** replace **ListCrafterQAgentChats** when the author wants **what was actually typed** in hosted chats. Injected **Current content item repository path** does **not** mean hosted-chat questions are about that file unless the user **explicitly** tied them together.
...
**CrafterQ hosted chat analytics:** If they ask what **people asked in CrafterQ**, **chat dislikes**, **top question from chats**, or **this agent**'s hosted conversation activity — and **ListCrafterQAgentChats** is in your tool list — **start** with **ListCrafterQAgentChats** ( `{}` is valid — server applies **last 30 days UTC** + session **agentId** ) or pass explicit **startDate**/**endDate**; then **GetCrafterQAgentChat** as needed. **Do not** open with **GetContent** or **ListContentTranslationScope** on the current preview path for that class of question.
...
For **content-only** tasks, use **`crafterqFormFieldUpdates`** in your final JSON. For **content type schema** changes, tell the author to edit the content type in Studio or use the preview assistant; do not imply a tool will commit the XML.''')
  ...
  static String getDESC_CONSULT_CRAFTERQ_EXPERT() {
    p('DESC_CONSULT_CRAFTERQ_EXPERT', 'Calls CrafterQ (api.crafterq.ai) as a subject-matter / content expert for the same agent as this chat: RAG-backed ideas for copy, tone, headings, SEO, IA, accessibility, and what to communicate on a page. Use when generating or refining author-facing content before writing to the CMS. Does NOT read or write repository files — use GetContent and WriteContent for that. **Not** for listing, ranking, or summarizing **past hosted chats** — use **ListCrafterQAgentChats** / **GetCrafterQAgentChat** when those are in the tool list and the author wants **what people asked** in conversation history. Required: question. Optional: context (audience, page goal, draft snippet, field you are filling). Injected tool-progress uses **🤓** after **🛠️**; include **🤓** in chat when you describe this consult.')
  }

  static String getDESC_LIST_CRAFTERQ_AGENT_CHATS() {
    p('DESC_LIST_CRAFTERQ_AGENT_CHATS', '**Primary tool** when the author asks what **people / users / authors asked in CrafterQ**, **hosted chat** activity, **dislikes**, **chat titles**, the "**top**" or "**number one**" **question in chat** for **this agent**, or "**what are people asking**" about the **CrafterQ agent** — **call this first**. Lists CrafterQ hosted chats via GET /v1/agents/{agentId}/chats. **Arguments:** You may pass **{}** (empty) — the server fills **startDate**/**endDate** as the **last 30 days UTC** and uses the session **agentId** from agent configuration. Or pass ISO-8601 UTC **startDate** and **endDate** (or only one: if only **endDate**, start = end minus 30 days; if only **startDate**, end = now UTC). Optional **agentId** overrides the session agent. Optional **limit** (1–100, default 20). **Do not** substitute **GetContent**, **ListContentTranslationScope**, **analyze_template**, or preview HTML for that intent. Returns **data** as the API JSON. On **HTTP 401**, the tool result includes **authHint** — tell the author to sign into CrafterQ in the widget (**X-CrafterQ-Chat-User**) or configure **crafterQBearerTokenEnv** / **crafterQBearerToken**. Same forwarded identity headers as hosted chat (**X-CrafterQ-Chat-User** when the author signed into CrafterQ in the widget).')
  }

  static String getDESC_GET_CRAFTERQ_AGENT_CHAT() {
    p('DESC_GET_CRAFTERQ_AGENT_CHAT', 'Loads one **hosted** CrafterQ conversation via GET /v1/agents/{agentId}/chats/{chatId} (messages and metadata). Use **after ListCrafterQAgentChats** when you need full threads to answer what was **actually asked** or how the agent replied. Required: **chatId**. Optional **agentId** (defaults to this widget session CrafterQ agent). Does not read or write the CMS repository.')
  }
