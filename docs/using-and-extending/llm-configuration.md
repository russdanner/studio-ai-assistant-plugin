...
| `<llm>` value | Aliases (normalize) | Behavior |
|----------------|---------------------|----------|
| **`crafterQ`** (default) | `crafter-q` | **Remote hosted chat** at `api.crafterq.ai` via `ExpertChatModel`. **Content / RAG style chat only** — the adapter does **not** run CMS tools. **Requires** `<crafterQAgentId>`. |
| **`openAI`** | `openai`, `open-ai` | **OpenAI** via Spring AI + **RestClient** native tool loop (`/v1/chat/completions`). |
| **`xAI`** | `x-ai`, `grok` | **xAI** (Grok): same OpenAI wire; base URL default `https://api.x.ai`. Keys: **`XAI_API_KEY`** or JVM **`crafter.xai.apiKey`**. Optional **`XAI_OPENAI_BASE_URL`** / **`crafter.xai.openAiBaseUrl`** (no trailing `/v1`). Default chat model JVM **`crafter.xai.model`** or `grok-2-latest` when `<llmModel>` is omitted. |
| **`deepSeek`** | `deep-seek` | **DeepSeek**: same wire; default base `https://api.deepseek.com`. Keys: **`DEEPSEEK_API_KEY`** / **`crafter.deepseek.apiKey`**. Default model **`crafter.deepseek.model`** or `deepseek-chat`. |
| **`llama`** | `ollama`, `meta-llama`, `meta_llama` | **OpenAI-compatible Llama** hosts (e.g. **Ollama**): default base `http://127.0.0.1:11434`. Keys: **`LLAMA_API_KEY`** / **`crafter.llama.apiKey`** (Ollama often accepts a placeholder). **`LLAMA_OPENAI_BASE_URL`**, **`OLLAMA_OPENAI_BASE_URL`**, or **`crafter.llama.openAiBaseUrl`**. Default model **`crafter.llama.model`** or `llama3.2`. |
| **`genesis`** / **`gemini`** | `gemini`, `google`, `google-genai`, `google_genai` | **Google Generative Language** OpenAI-compatible endpoint (default base `https://generativelanguage.googleapis.com/v1beta/openai`). Keys: **`GEMINI_API_KEY`**, **`GOOGLE_API_KEY`**, or JVM **`crafter.gemini.apiKey`** / **`crafter.google.apiKey`**. Default model **`crafter.gemini.model`** or `gemini-2.0-flash`. |
| **`claude`** | `anthropic` | **Anthropic Claude** via Spring AI (`AnthropicChatModel`). Native CMS tools run **inside Spring AI** (not the OpenAI RestClient loop). Keys: **`ANTHROPIC_API_KEY`** / **`crafter.anthropic.apiKey`**. Default model **`crafter.anthropic.model`** or `claude-3-5-sonnet-20241022`. |
| **`script:{id}`** | — | **Site Groovy LLM** — see **Site script LLM** below. `{id}` = lowercase `a-z`, `0-9`, `_`, `-`, max 64. |

Optional **`<llmModel>`** is sent as the provider’s chat model id. For **`openAI`**, if omitted, the server uses JVM **`crafter.openai.model`** (see existing OpenAI docs). Other providers use the JVM defaults in the table row when `<llmModel>` is blank.

Optional **`<imageModel>`** (e.g. `dall-e-3`, `dall-e-2`) selects the **OpenAI Images API** model for the **`GenerateImage`** tool. **GenerateImage**, **QueryExpertGuidance** embeddings, and **ConsultCrafterQExpert** still use **`OPENAI_API_KEY`** (or JVM / widget OpenAI key) where applicable — they are not switched by `<llm>` alone.

### Site script LLM (`script:{id}`)

Use **`&lt;llm&gt;script:mybackend&lt;/llm&gt;`** (normalized **`scriptLlm:mybackend`**) and add Groovy under the site Studio scripts tree:

- **Repo path:** `config/studio/scripts/aiassistant/llm/mybackend/runtime.groovy` (or `llm.groovy`).
- **Configuration service path (module `studio`):** `/scripts/aiassistant/llm/mybackend/runtime.groovy`.

The script is evaluated with binding variables **`log`**, **`llmId`**, **`siteId`**, **`scriptPath`**. It must evaluate to either **`StudioAiLlmRuntime`** or a **Map** with **`supportsNativeStudioTools`** (boolean) and **`buildSessionBundle`** (one-arg **Closure** taking **`StudioAiRuntimeBuildRequest`** and returning the same session **Map** as the built-in OpenAI or Anthropic runtimes). For OpenAI-wire + RestClient CMS tools, include **`openAiWireBaseUrl`** (no trailing `/v1`), **`resolvedChatModel`**, **`openAiApiKeyResolved`**, **`chatClient`**, **`tools`**, **`useTools`**, **`studioOps`**. For Anthropic-style streaming tools, return an Anthropic **`chatClient`** and set **`nativeToolTransport`** to **`anthropic`** on the map. The container forces **`llm`** to **`scriptLlm:{id}`** in the bundle for logging while still using your wire fields for routing.

**Security:** Same as **`user-tools/`** — committers to this path run code in the Studio JVM as the authenticated author.

**Example (proxy OpenAI):** `docs/examples/aiassistant-llm/demo/runtime.groovy`.
...
### CrafterQ API tools on the OpenAI path (`ConsultCrafterQExpert`, `ListCrafterQAgentChats`, `GetCrafterQAgentChat`)
...
1. **`<llm>`** — use a **tool-capable** value from the table above (**not** `crafterQ` if you need these tools).
...
3. **Tools enabled for the request** — same as other CMS tools: do **not** use **`omitTools: true`** for turns where the model should call them; ensure **`&lt;enableTools&gt;false&lt;/enableTools&gt;`** is **not** set on the agent if you want tools at all (see **[spec.md](../internals/spec.md)** / agent XML for `enableTools`).
...
- `llm`: `crafterQ` | `openAI` | `xAI` | `deepSeek` | `llama` | `genesis` | `gemini` | `claude` | `script:{id}` (optional; default `crafterQ`). Matching aliases are normalized server-side (e.g. `grok` → xAI, `ollama` → llama). **`script:myid`** → **`scriptLlm:myid`** and loads site Groovy from `/scripts/aiassistant/llm/myid/runtime.groovy`.
...
Additional hosted-tool contracts or provider rows may be documented here as they ship. **`ConsultCrafterQExpert`** already calls the hosted stack as a **CMS tool** from OpenAI-wire sessions.
