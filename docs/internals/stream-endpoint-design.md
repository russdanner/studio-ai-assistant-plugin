...
- **LLM selection**: Widget configuration (same shape as `ui.xml`) sets per-agent **`<llm>crafterQ</llm>`** or **`<llm>openAI</llm>`**; the UI sends `llm` (and optional **`llmModel`** / **`imageModel`**) on each stream/chat request. See **[llm-configuration.md](../using-and-extending/llm-configuration.md)**.
