...
- **`elementXpath`** — XPath used to detect an existing plugin widget (avoids duplicate inserts). Should uniquely identify your plugin’s widget (e.g. by widget id and plugin id).
...
- **Preview Toolbar (top bar):** Under `craftercms.components.PreviewToolbar` → `configuration` → `middleSection` (or left/right) → `widgets` → your widget.
...
Field reference, REST endpoints, `control` actions, and human-task behavior: **`docs/SPEC.md`** § *Autonomous assistants widget (Tools Panel)*. The marketplace/descriptor sample merges this under **Tools Panel**; custom sites can paste the block manually into **`config/studio/ui.xml`**.
...
- `craftercms.components.PreviewToolbar` → `configuration` → `middleSection` → `widgets`
...
- If you added a **toolbar** entry in the descriptor, the toolbar widget should appear after install. If not, add the widget manually to `PreviewToolbar` → `middleSection` → `widgets` (see section 3).
