...
| **`openAiApiKey`** | no | Same testing-only semantics as Helper agents — only when no server/JVM key; see **[llm-configuration.md](../using-and-extending/llm-configuration.md)**. |
...
- **agent.imageModel** — Optional OpenAI **Images** model id for the **GenerateImage** tool (e.g. `gpt-image-1` or `gpt-image-1-mini`). Obsolete `dall-e-*` strings from older configs map to `gpt-image-1` server-side. ui.xml **`<imageModel>`**; stream/chat JSON **`imageModel`**. If omitted or blank, **`GenerateImage`** fails until configured — **no** server or JVM fallback.
