...
| **Request body** | JSON: `{ "agentId": "<uuid>", "prompt": "<full prompt>", "chatId": "<optional>", "llm": "crafterQ \| openAI", "llmModel": "<optional>", "imageModel": "<optional OpenAI Images model id; required for GenerateImage when no agent imageModel (use gpt-image-1 family; DALL·E image models retired 2026-05-12)>", "openAiApiKey": "<optional testing>" }` |
...
- **LLM selection**: Per-agent **`&lt;llm&gt;`** in `ui.xml` (or widget JSON). The client sends **`llm`** on the stream/chat POST only when set; if omitted, the server normalizes missing/blank/unknown values—see **[llm-configuration.md](../using-and-extending/llm-configuration.md)** (table + **Omitted `llm` on the stream/chat POST**). See also **`llmModel`** / **`imageModel`** on each request.
- **CrafterQ (`llm` normalizes to `crafterQ`)**: Spring AI **`ExpertChatModel`** POSTs a **single text `prompt`** to CrafterQ’s `/v1/chats` API. **No CMS tools** on this path — content/RAG style chat only. Prompt = short system text + `Human:` / `Assistant:` transcript. **DEBUG logs** (when enabled): `CrafterQ HTTP TX/RX` previews, `CrafterQ call start/parsed` (see **README.md** server logging).
