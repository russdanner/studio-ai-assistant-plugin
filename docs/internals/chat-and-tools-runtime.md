...
- **`GenerateImage` (OpenAI only):** Calls **`POST /v1/images/generations`** with the same API key as chat. The image model is **only** from the agent **`<imageModel>`** (widget JSON **`imageModel`**) or the stream/chat POST body **`imageModel`** — there is **no** JVM or legacy-key fallback; if omitted or blank, image generation errors until configured. **OpenAI retired the DALL·E 2 / DALL·E 3 Images API on 2026-05-12**; configure a **GPT Image** family id supported for your account (for example **`gpt-image-1`** or **`gpt-image-1-mini`**) and confirm **`size`** / **`quality`** with current OpenAI documentation. Returned image URLs are short-lived; for the site, download into **`/static-assets/`** and reference that path.
...
- `imageModel`: optional string — OpenAI **Images** model id for **GenerateImage**; must be set on the agent and/or this body field when the model should call **GenerateImage** (no server default). Prefer **GPT Image** ids (e.g. **`gpt-image-1`**); legacy **dall-e-2** / **dall-e-3** were removed from this API on **2026-05-12** per OpenAI.
